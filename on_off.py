...
def file_name(spectrum_type: SpectrumType, gain: int, integration_time: float) -> str:
    """
    ...
    return f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{spectrum_type.value}_{gain}db_{integration_time}.npy"
...
if __name__ == "__main__":
    with RTLSDR(integration_time=INTEGRATION_TIME, gain=GAIN) as rtl:
        ...
        off_filename = file_name(SpectrumType.OFF, GAIN, INTEGRATION_TIME)
        ...
        freqs, powers, overhead_time = rtl.take_exposure()
        on_filename = file_name(SpectrumType.ON, GAIN, INTEGRATION_TIME)
